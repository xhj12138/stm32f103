...
<<<<<<< HEAD
    ...
    else if (huart->Instance == BT04_UART_INTERFACE)            /* 如果是串口3，进行串口3 MSP初始化 */
    {
        BT04_UART_TX_GPIO_CLK_ENABLE();                         /* 使能串口TX脚时钟 */
        BT04_UART_RX_GPIO_CLK_ENABLE();                         /* 使能串口RX脚时钟 */
        BT04_UART_CLK_ENABLE();                                 /* 使能串口时钟 */

        gpio_init_struct.Pin = BT04_UART_TX_GPIO_PIN;           /* 串口发送引脚号 */
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* IO速度设置为高速 */
        HAL_GPIO_Init(BT04_UART_TX_GPIO_PORT, &gpio_init_struct);
                
        gpio_init_struct.Pin = BT04_UART_RX_GPIO_PIN;           /* 串口RX脚 模式设置 */
        gpio_init_struct.Mode = GPIO_MODE_AF_INPUT;             /* 串口RX脚 必须设置成输入模式 */
        HAL_GPIO_Init(BT04_UART_RX_GPIO_PORT, &gpio_init_struct);   
        
        HAL_NVIC_EnableIRQ(BT04_UART_IRQn);                      /* 使能蓝牙串口中断通道 */
        HAL_NVIC_SetPriority(BT04_UART_IRQn, 0, 1);              /* 组0，最高优先级:抢占优先级0，子优先级1 */
